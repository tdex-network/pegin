...
```
git submodule update --init && cd libwally-core && git submodule update --init
make build
```
...
const peginModule = new ElementsPegin([
  ...
  ElementsPegin.withFederationScript(
...
- `main.wasm` is compiled from wrappers in `main.go` using `resources/Dockerfile.goelements`.
